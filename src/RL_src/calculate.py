...
global_path = "/home/gaoucsd/catkin_ws/src/mavros_controller/geometric_controller/nodes/trained_models/"
env_name = "QuadRotor-v0"
seed = 0
algo = "ppo"
# loadfile = "up_11" #takeoff works
# loadfile = "up_11_t2" #best takeoff 
loadfile = "Takeoff"
...
class callback:
    def __init__(self, actor_critic):
        ...
        ### Sub / Pub
        rospy.Subscriber("reference/flatsetpoint", FlatTarget, self.ref_cb)
        ...
        self.pubCmd = rospy.Publisher("RLcmd", AttitudeTarget, queue_size=10)
        ...
        self.rotmat = np.array((3, 3))
        self.rotmat_d = np.array((3, 3))
        self.euler = np.array((3,))
        self.euler_d = np.array((3,))
    ...
    def mainLoop(self, timer):
        ...
        while True:
            ...
            with torch.no_grad():
                value, self.action, _, recurrent_hidden_states = self.actor_critic.act(
    ...
    def getError(self):
        ...
        obs = torch.from_numpy(npdata).float()
        return obs
    ...
    def PDcontroller(self):
        ...
        self.M2 = jy* KdM[1][0]* (self.pitch_v- 0) + KpM[1][0]* ( theta_des - self.pitch)
        ...
        # Controller [F, M1, M2, M3]   
    def cmdConversion(self):

        action = self.action.numpy()[0, 0, 0]
        # print("action", action)
        o1, o2, o3, o4 = action[0], action[1], action[2], action[3]
        ...
        self.U1 = b * (o1 ** 2 - o2 ** 2)
        self.U2 = b * (o3 ** 2 - o1 ** 2)
        self.U3 = d * (o1 ** 2 + o3 ** 2 - o2 ** 2 - o4 ** 2)
        self.Omega = p * (o2 + o4 - o1 - o3)

        print([self.U1, self.U2, self.U3, self.Omega])
...
def main():
    ...
    if loadfile:
        rospy.loginfo(rospy.get_caller_id() + "get the Trained model!")
        # model_path = os.path.join('/trained_models/', algo, env_name + loadfile + '.pt')
        actor_critic, ob_rms = torch.load(
            global_path + algo + "/" + env_name + loadfile + ".pt"
        )
    else:
        rospy.loginfo("No Pretrained model loaded")

    rospy.init_node("calculate", anonymous=True)
    rospy.loginfo("Started!")

    cb = callback(actor_critic)
